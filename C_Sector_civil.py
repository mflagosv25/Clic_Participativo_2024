...
def get_sector(texto,diccionario):
    ...
    if sector_found == "": sector_found = "no aplica"
    return sector_found
...
def organize_actores(data):
    ...
    act_unicos_ini = data.drop_duplicates(subset = ['Actores'], keep='first').copy()
    return act_unicos_ini

def select_columns(df_act_conteo):
    df = df_act_conteo[["ID_Actor","Actores_limpio"]].copy()
...
if __name__ == '__main__':
    ...
    data = select_columns(df_act_conteo)
